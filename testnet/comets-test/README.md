...
>Prerequisite: go1.17+ required. [ref](https://golang.org/doc/install)
   ...
   Append the below lines to the file `${HOME}/.bashrc` 
   ...
   ```
   Execute the command `source ${HOME}/.bashrc` to reflect in the current Terminal session

>Prerequisite: git. [ref](https://github.com/git/git)

>Optional requirement: GNU make. [ref](https://www.gnu.org/software/make/manual/html_node/index.html)

>Optional requirement: jq [ref](https://github.com/stedolan/jq)
...
## For new users -> fullnode / validator setup, [click here](#create-fullnode-after-gentx-for-new-users)

~~## Validator setup instructions before GenTx  **(OLD SETUP, IGNORE)**~~
...
## Create fullnode after GenTx (For new users)
...
* Check if the node is fully synced to the latest block height of the chain.  
```shell
comdex status | jq
``` 
Only if ```'catching_up':false``` -  Node is fully synced and it can be used for Validator 
...
* [TestNet Explorer](https://comets-test.comdex.one/)
